...
sub new($$$$$$) {
	...
	if($fqn =~ m|\bXPP_(\w+)/.*$|) {
	...
	} elsif ($fqn =~ m{\bFXO200M/.*}) {
		$type = "FXO"; # OpenVox fxo200m module span
	} elsif ($fqn =~ m{\bFXS200M/.*}) {
		$type = "FXS"; # OpenVox fxs200m module span
	} elsif ($fqn =~ m{\bB200M1/.*}) {
		$type = "BRI"; # OpenVox b200m module span
	} elsif ($fqn =~ m{---/.*}) {
		...
		# D100M: OpenVox D100M module span card driver
		$type = "PRI";
...
sub probe_type($) {
	...
	# Fix the Regular match mode in higher perl version, SantMoriz Chiu, 2019-03-29 17:33 
	#if($fqn =~ m:WCTDM/|WRTDM/|OPVXA1200/|OPVXA24XX/|FXS200M/|FXO200M:) {
	if($fqn =~ m:(WCTDM|WRTDM|OPVXA1200|OPVXA24XX|FXS200M|FXO200M):) {
		my %maybe;

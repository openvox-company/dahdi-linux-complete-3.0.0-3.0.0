...
my @bri_strings = (
        ...
        '(B200M) \(NT|TE\)', # has selectable NT/TE modes via dahdi_cfg
		);
...
sub new($$) {
	...
	if($self->is_bri()) {
		...
		if($chan_fqn =~ m(ZTHFC.*/|ztqoz.*/|XPP_BRI_.*|B4/.*|WCBRI/.*|B200M1/.*)) {		# BRI
